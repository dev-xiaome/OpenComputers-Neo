        ...
        try sources.synchronized(sources += new Source(x, y, z, data, gain)) catch {
          case e: OpenALException =>
            if (e.errorCode == AL10.AL_OUT_OF_MEMORY) {
              ...
              OpenComputersNeo.log.info("Couldn't play computer speaker sound because your sound card ran out of memory. Either your sound card is just really low-end, or there are just too many sounds in use already by other mods. Disabling computer speakers to avoid spamming your log file now.")
              disableAudio = true
            }
            else {
              OpenComputersNeo.log.warn("Error playing computer speaker sound.", e)
            }
        }
      }
    }
      ...
      runOnSoundEngine {
        ...
        try AL10.alGetError() catch {
          case _: UnsatisfiedLinkError =>
            OpenComputersNeo.log.warn("Negotiations with OpenAL broke down, disabling sounds.")
            disableAudio = true
        }
      }
    }
