...
object EventHandler {
  ...
  def scheduleFMP(tileEntity: () => BlockEntity) {
    if (SideTracker.isServer) pendingServer.synchronized {
      pendingServer += (() => Network.joinOrCreateNetwork(tileEntity()))
    }
  }
  ...
  def scheduleAE2Add(tileEntity: power.AppliedEnergistics2): Unit = {
    if (SideTracker.isServer) pendingServer.synchronized {
      pendingServer += (() => if (!tileEntity.isInvalid) {
        tileEntity.getGridNode(Direction.UNKNOWN).updateState()
      })
    }
  }
  ...
  def onCrafting(e: ItemCraftedEvent) = {
    ...
    didRecraft = recraft(e, navigationUpgrade, stack => {
      ...
      Option(api.Driver.driverFor(e.crafting)) match {
        case Some(driver) => Option(ItemStack.loadItemStackFromNBT(driver.dataTag(stack).getCompound(Settings.namespace + "map")))
        case _ => None
      }
    }) || didRecraft
    ...
    e.player match {
      ...
      case player: ServerPlayer if player.getEntityWorld != null && !player.getEntityWorld.isRemote =>
