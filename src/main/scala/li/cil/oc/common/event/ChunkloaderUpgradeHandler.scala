...
object ChunkloaderUpgradeHandler {
  ...
  def onRegisterTicketControllers(event: RegisterTicketControllersEvent): Unit = {
    ticketController = new TicketController(
      ResourceLocation.fromNamespaceAndPath(OpenComputersNeo.ID, "chunkloader"),
    ...
    for ((owner, ticketSet) <- helper.getEntityTickets) {
      ...
      if (tickets.size == 9) {
        ...
        if (maxX - minX == 2 && maxZ - minZ == 2 && positions.toSet == expected) {
          val x = minX + 1
          val z = minZ + 1
          OpenComputersNeo.log.info(s"Restoring chunk loader ticket for upgrade at chunk ($x, $z) with address ${owner}.")
          restoredTickets += owner -> RestoredTicket(world.dimension, new ChunkPos(x, z))
        } else {
          OpenComputersNeo.log.warn(s"Chunk loader ticket for $owner loads an incorrect shape.")
          helper.removeAllTickets(owner)
          restoredTickets.remove(owner)
        }
      } else {
        OpenComputersNeo.log.warn(s"Chunk loader ticket for $owner loads ${tickets.size} chunks.")
        helper.removeAllTickets(owner)
        restoredTickets.remove(owner)
      }
    }
  ...
  def onWorldSave(e: LevelEvent.Save): Unit = e.getLevel match {
    case level: ServerLevel =>
      ...
      for ((owner, ticket) <- orphaned) {
        try {
          OpenComputersNeo.log.warn(s"A chunk loader ticket has been orphaned! Address: ${owner}, position: (${ticket.pos.x}, ${ticket.pos.z}) in ${ticket.dimension.location}. Removing...")
          releaseTicket(level, owner.toString, ticket.pos)
        } catch {
          case err: Throwable => OpenComputersNeo.log.error(err)
        } finally {
          restoredTickets.remove(owner)
        }
      }
  ...
  def releaseTicket(level: ServerLevel, addr: String, pos: ChunkPos): Unit = parseAddress(addr) match {
    ...
    case _ => OpenComputersNeo.log.warn(s"Address '$addr' could not be parsed")
  }
