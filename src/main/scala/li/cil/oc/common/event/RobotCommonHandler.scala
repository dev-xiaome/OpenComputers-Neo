  ...
  def onRobotApplyDamageRate(e: RobotUsedToolEvent.ApplyDamageRate): Unit = {
    if (e.toolAfterUse.isItemStackDamageable) {
      ...
      if (damage > 0) {
        ...
        e.toolAfterUse.setDamageValue(e.toolAfterUse.getItemDamage - repairedDamage)
      }
    }
  }
  ...
  def onRobotMove(e: RobotMoveEvent.Pre): Unit = {
    if (Settings.get.limitFlightHeight >= 0) e.agent match {
      case robot: Robot =>
        ...
        def hasAdjacentBlock(pos: BlockPosition) = Direction.VALID_DIRECTIONS.exists(side => world.isSideSolid(pos.offset(side), side.getOpposite))
