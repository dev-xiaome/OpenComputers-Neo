...
abstract class Player(val playerInventory: Inventory, val otherInventory: IInventory) extends Container {
  /** Number of player inventory slots to display horizontally. */
  protected val playerInventorySizeX = math.min(9, Inventory.getHotbarSize)
  ...
  override def canInteractWith(player: Player) = otherInventory.isUseableByPlayer(player)

  override def slotClick(slot: Int, mouseClick: Int, holdingShift: Int, player: Player) = {
  ...
  override def transferStackInSlot(player: Player, index: Int): ItemStack = {
  ...
  def updateCustomData(nbt: CompoundTag): Unit = {
    if (nbt.contains("delta")) {
      ...
      delta.func_150296_c().foreach {
        case key: String => synchronizedData.put(key, delta.getTag(key))
      }
    }
  }
  ...
  protected class SynchronizedData extends CompoundTag {
    ...
    override def setTag(key: String, value: Tag): Unit = this.synchronized {
      if (!value.equals(getTag(key))) delta.put(key, value)
      super.put(key, value)
    }
    ...
    override def setInteger(key: String, value: Int): Unit = this.synchronized {
      if (value != getInteger(key)) delta.putInt(key, value)
