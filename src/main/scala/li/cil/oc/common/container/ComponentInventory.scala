...
import li.cil.oc.integration.OpenComputersNeo.{DriverTablet, Item}
...
trait ComponentInventory extends Inventory with network.Environment {
            ...
            Option(driver.createEnvironment(stack, host)) match {
              case Some(component) =>
                ...
                try {
                  ...
                  case e: Throwable => OpenComputersNeo.log.warn(s"An item component of type '${component.getClass.getName}' (provided by driver '${driver.getClass.getName}') threw an error while loading.", e)
                }
    ...
    for (slot <- 0 until getContainerSize) {
      ...
      if (!stack.isEmpty) {
        if (slot >= componentSlots.length) {
          ...
          // nbt data had been parsed. See https://github.com/MightyPirates/OpenComputersNeo/issues/2522
          // If this error is hit again, perhaps another subtype needs to handle nbt loading like Case does
          OpenComputersNeo.log.error(s"ComponentInventory components length ${componentSlots.length} does not accommodate inventory size ${getContainerSize}")
          return
        } else {
  ...
  protected def save(component: ManagedEnvironment, driver: ItemDriver, stack: ItemStack): Unit = {
    try {
      ...
      case e: Throwable => OpenComputersNeo.log.warn(s"An item component of type '${component.getClass.getName}' (provided by driver '${driver.getClass.getName}') threw an error while saving.", e)
    }
  }
