...
trait RedstoneAware extends RotationAware with IConnectable with IRedstoneEmitter with IRedstoneUpdatable {
  ...
  def getOutput: Array[Int] = Direction.VALID_DIRECTIONS.map{ side: Direction => _output(toLocal(side).ordinal) }

  def getOutput(side: Direction) = Option(_output) match {
  ...
  override def updateEntity(): Unit = {
    ...
    if (isServer) {
      if (shouldUpdateInput) {
        ...
        Direction.VALID_DIRECTIONS.foreach(updateRedstoneInput)
      }
    }
  }
  ...
  override def validate(): Unit = {
    ...
    if (!canUpdate && isServer) {
      EventHandler.scheduleServer(() => Direction.VALID_DIRECTIONS.foreach(updateRedstoneInput))
    }
  }
  ...
  @Optional.Method(modid = Mods.IDs.RedLogic)
  override def getEmittedSignalStrength(blockFace: Int, toDirection: Int): Short = _output(toLocal(Direction.getOrientation(toDirection)).ordinal()).toShort
