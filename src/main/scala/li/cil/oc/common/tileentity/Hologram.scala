...
class Hologram(pos: BlockPos, state: BlockState)
  ...
  override def writeToNBTForServer(nbt: CompoundTag): Unit = this.synchronized {
    ...
    nbt.putDouble(Settings.namespace + "offsetZ", translation.zCoord)
  ...
  override def writeToNBTForClient(nbt: CompoundTag): Unit = {
    ...
    nbt.putDouble("offsetX", translation.xCoord)
    nbt.putDouble("offsetY", translation.yCoord)
    nbt.putDouble("offsetZ", translation.zCoord)
