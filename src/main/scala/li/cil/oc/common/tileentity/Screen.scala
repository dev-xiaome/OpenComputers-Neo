...
class Screen(var tier: Int) extends traits.TextBuffer with SidedEnvironment with traits.Rotatable with traits.RedstoneAware with traits.Colored with Analyzable with Ordered[Screen] {
  ...
  override def canConnect(side: Direction) = toLocal(side) != Direction.SOUTH

  // Allow connections from front for keyboards, and keyboards only...
  override def sidedNode(side: Direction) = if (toLocal(side) != Direction.SOUTH || (world.blockExists(position.offset(side)) && world.getTileEntity(position.offset(side)).isInstanceOf[Keyboard])) node else null
  ...
  def hasKeyboard = screens.exists(screen =>
    Direction.VALID_DIRECTIONS.map(side => (side, {
  ...
  def toScreenCoordinates(hitX: Double, hitY: Double, hitZ: Double): (Boolean, Option[(Double, Double)]) = {
    ...
    def dot(f: Direction) = f.offsetX * hitX + f.offsetY * hitY + f.offsetZ * hitZ
    ...
    if (arrows.nonEmpty) {
      for (arrow <- arrows) {
        ...
        arrow.shootingEntity match {
          case player: Player if player == Minecraft.getMinecraft.thePlayer => click(hitX, hitY, hitZ)
          case _ =>
        }
      }
      arrows.clear()
  ...
  override def getRenderBoundingBox =
    if ((width == 1 && height == 1) || !isOrigin) super.getRenderBoundingBox
      ...
      case _ =>
        ...
        val b = AABB.getBoundingBox(ox, oy, oz, ox + sx, oy + sy, oz + sz)
  ...
  override def onAnalyze(player: Player, side: Int, hitX: Float, hitY: Float, hitZ: Float) = Array(origin.node)
  ...
  private def project(t: Screen) = {
    def dot(f: Direction, s: Screen) = f.offsetX * s.x + f.offsetY * s.y + f.offsetZ * s.z
  ...
  private def unproject(x: Int, y: Int, z: Int) = {
    def dot(f: Direction) = f.offsetX * x + f.offsetY * y + f.offsetZ * z
