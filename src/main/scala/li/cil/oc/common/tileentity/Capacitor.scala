...
class Capacitor extends traits.Environment with DeviceInfo {
  ...
  def recomputeCapacity(updateSecondGradeNeighbors: Boolean = false) {
    node.setLocalBufferSize(
      Settings.get.bufferCapacitor +
        Settings.get.bufferCapacitorAdjacencyBonus * Direction.VALID_DIRECTIONS.count(side => {
  ...
  private def indirectNeighbors = Direction.VALID_DIRECTIONS.map(side => (x + side.offsetX * 2, y + side.offsetY * 2, z + side.offsetZ * 2))
