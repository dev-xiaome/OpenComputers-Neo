...
class NeuralNetwork(controller: ControllerImpl) extends Persistable {
  ...
  override def load(nbt: CompoundTag): Unit = {
    triggers.clear()
    nbt.getList("triggers", NBT.TAG_COMPOUND).foreach((t: CompoundTag) => {
    ...
    connectors.clear()
    nbt.getList("connectors", NBT.TAG_COMPOUND).foreach((t: CompoundTag) => {
    ...
    behaviors.clear()
    nbt.getList("behaviors", NBT.TAG_COMPOUND).foreach((t: CompoundTag) => {
      api.Nanomachines.getProviders.find(p => p.readFromNBT(controller.player, t.getCompound("behavior")) match {
        case b: Behavior =>
