  ...
  override def writeBehaviorToNBT(behavior: Behavior, nbt: CompoundTag): Unit = {
    behavior match {
      case particles: ParticleBehavior =>
        nbt.putInt("effectName", BuiltInRegistries.PARTICLE_TYPE.asInstanceOf[ForgeRegistry[ParticleType[_]]].getID(particles.effectType))
      case _ => // Wat.
    }
  }

  override def readBehaviorFromNBT(player: Player, nbt: CompoundTag): Behavior = {
    val effectType = BuiltInRegistries.PARTICLE_TYPE.asInstanceOf[ForgeRegistry[ParticleType[_]]].getValue(nbt.getInt("effectName"))
