...
object ParticleProvider extends ScalaProvider("b48c4bbd-51bb-4915-9367-16cff3220e4b") {
  ...
  override def writeBehaviorToNBT(behavior: Behavior, nbt: CompoundTag): Unit = {
    behavior match {
      case particles: ParticleBehavior =>
        nbt.putInt("effectName", BuiltInRegistries.PARTICLE_TYPES.asInstanceOf[ForgeRegistry[ParticleType[_]]].getID(particles.effectType))
      case _ => // Wat.
    }
  }

  override def readBehaviorFromNBT(player: Player, nbt: CompoundTag): Behavior = {
    val effectType = BuiltInRegistries.PARTICLE_TYPES.asInstanceOf[ForgeRegistry[ParticleType[_]]].getValue(nbt.getInt("effectName"))
  ...
  class ParticleBehavior(var effectType: SimpleParticleType, player: Player) extends AbstractBehavior(player) {
    override def getNameHint = "particles." + BuiltInRegistries.PARTICLE_TYPES.getKey(effectType).getPath
