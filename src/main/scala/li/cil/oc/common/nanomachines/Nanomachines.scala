...
object Nanomachines extends api.detail.NanomachinesAPI {
  ...
  def controllers(player: Player) = if (player.getEntityWorld.isRemote) clientControllers else serverControllers
