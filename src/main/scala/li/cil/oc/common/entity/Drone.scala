...
class Drone(val world: Level) extends Entity(world) with MachineHost with internal.Drone with internal.Rotatable with Analyzable with Context {
  ...
  val components = new ComponentInventory {
    ...
    override def isUseableByPlayer(player: Player) = true
  ...
  override def toLocal(value: Direction) = value

  override def toGlobal(value: Direction) = value
  ...
  override def onUpdate(): Unit = {
    ...
    if (isRunning) {
      ...
      val groundDrag = worldObj.getBlock(BlockPosition(this: Entity).offset(Direction.DOWN)).slipperiness * drag
      motionX *= groundDrag
      motionY *= drag
      motionZ *= groundDrag
      if (onGround) {
  ...
  override def interactFirst(player: Player): Boolean = {
  ...
  override def kill(): Unit = {
    ...
    if (!world.isRemote) {
      ...
      info.save(stack)
      val entity = new ItemEntity(world, posX, posY, posZ, stack)
  ...
  override def readEntityFromNBT(nbt: CompoundTag): Unit = {
    info.load(nbt.getCompound("info"))
    inventorySize = computeInventorySize()
    if (!world.isRemote) {
      machine.load(nbt.getCompound("machine"))
      control.load(nbt.getCompound("control"))
  ...
  override def writeEntityToNBT(nbt: CompoundTag): Unit = {
