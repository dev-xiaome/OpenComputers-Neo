...
class PrintData extends ItemData(Constants.BlockName.Print) {
  ...
  override def load(nbt: CompoundTag): Unit = {
    if (nbt.contains("label")) label = Option(nbt.getString("label")) else label = None
    if (nbt.contains("tooltip")) tooltip = Option(nbt.getString("tooltip")) else tooltip = None
    ...
    stateOff.clear()
    stateOff ++= nbt.getList("stateOff", NBT.TAG_COMPOUND).map(PrintData.nbtToShape)
    stateOn.clear()
    stateOn ++= nbt.getList("stateOn", NBT.TAG_COMPOUND).map(PrintData.nbtToShape)
...
object PrintData {
  ...
  def computeApproximateOpacity(shapes: Iterable[PrintData.Shape]) = {
    ...
    if (shapes.nonEmpty) for (x <- 0 until 16 / stepping; y <- 0 until 16 / stepping; z <- 0 until 16 / stepping) {
      val bounds = AABB.getBoundingBox(
  ...
  def nbtToShape(nbt: CompoundTag): Shape = {
    val aabb =
      if (nbt.contains("minX")) {
        ...
        AABB.getBoundingBox(minX, minY, minZ, maxX, maxY, maxZ)
      }
      else {
    ...
    val tint = if (nbt.contains("tint")) Option(nbt.getInteger("tint")) else None
