  ...
  def analyze(thing: AnyRef, player: Player, side: Int, hitX: Float, hitY: Float, hitZ: Float): Boolean = {
    ...
    thing match {
      ...
      case host: SidedEnvironment =>
        if (!world.isRemote) {
          analyzeNodes(Array(host.sidedNode(Direction.getOrientation(side))), player)
        }
        true
...
class Analyzer(val parent: Delegator) extends traits.Delegate {
  override def onItemRightClick(stack: ItemStack, world: Level, player: Player): ItemStack = {
    if (player.isSneaking && stack.hasTagCompound) {
      stack.getTagCompound.remove(Settings.namespace + "clipboard")
  ...
  override def onItemUse(stack: ItemStack, player: Player, position: BlockPosition, side: Int, hitX: Float, hitY: Float, hitZ: Float) = {
    val world = player.getEntityWorld
    world.getTileEntity(position) match {
      case screen: tileentity.Screen if Direction.getOrientation(side) == screen.facing =>
        if (player.isSneaking) {
          screen.copyToAnalyzer(hitX, hitY, hitZ)
        }
        else if (stack.hasTagCompound && stack.getTagCompound.contains(Settings.namespace + "clipboard")) {
          if (!world.isRemote) {
