  ...
  def handleEvent(e: IMCEvent): Unit = {
    for (message <- e.getMessages) {
      if (message.key == "registerAssemblerTemplate" && message.isNBTMessage) {
        if (message.getNBTValue.contains("name", NBT.TAG_STRING))
        ...
        try Wrench.addUsage(getStaticMethod(message.getStringValue, classOf[Player], classOf[Int], classOf[Int], classOf[Int], classOf[Boolean])) catch {
        ...
        try Registry.blacklistHost(ItemStack.loadItemStackFromNBT(message.getNBTValue.getCompound("item")), Class.forName(message.getNBTValue.getString("host"))) catch {
