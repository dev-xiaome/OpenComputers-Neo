...
object IMC {
    ...
    message.messageSupplier().get.asInstanceOf[AnyRef] match {
      case template: CompoundTag if message.method == api.IMC.REGISTER_ASSEMBLER_TEMPLATE => {
        if (template.contains("name", Tag.TAG_STRING))
          OpenComputersNeo.log.debug(s"Registering new assembler template '${template.getString("name")}' from mod ${message.senderModId}.")
        else
          OpenComputersNeo.log.debug(s"Registering new, unnamed assembler template from mod ${message.senderModId}.")
        try AssemblerTemplates.add(template) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering assembler template.", t)
        }
      }
      case template: CompoundTag if message.method == api.IMC.REGISTER_DISASSEMBLER_TEMPLATE => {
        if (template.contains("name", Tag.TAG_STRING))
          OpenComputersNeo.log.debug(s"Registering new disassembler template '${template.getString("name")}' from mod ${message.senderModId}.")
        else
          OpenComputersNeo.log.debug(s"Registering new, unnamed disassembler template from mod ${message.senderModId}.")
      ...
      case name: String if message.method == api.IMC.REGISTER_TOOL_DURABILITY_PROVIDER => {
        OpenComputersNeo.log.debug(s"Registering new tool durability provider '${name}' from mod ${message.senderModId}.")
        try ToolDurabilityProviders.add(getStaticMethod(name, classOf[ItemStack])) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering tool durability provider.", t)
        }
      }
      case name: String if message.method == api.IMC.REGISTER_WRENCH_TOOL => {
        OpenComputersNeo.log.debug(s"Registering new wrench usage '$name' from mod ${message.senderModId}.")
        try Wrench.addUsage(getStaticMethod(name, classOf[Player], classOf[BlockPos], classOf[Boolean])) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering wrench usage.", t)
        }
      }
      case name: String if message.method == api.IMC.REGISTER_WRENCH_TOOL_CHECK => {
        OpenComputersNeo.log.debug(s"Registering new wrench tool check '$name' from mod ${message.senderModId}.")
        try Wrench.addCheck(getStaticMethod(name, classOf[ItemStack])) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering wrench check.", t)
        }
      }
      case implInfo: CompoundTag if message.method == api.IMC.REGISTER_ITEM_CHARGE => {
        OpenComputersNeo.log.debug(s"Registering new item charge implementation '${implInfo.getString("name")}' from mod ${message.senderModId}.")
        try ItemCharge.add(
          ...
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering item charge implementation.", t)
        }
      }
      case name: String if message.method == api.IMC.BLACKLIST_PERIPHERAL => {
        OpenComputersNeo.log.debug(s"Blacklisting CC peripheral '$name' as requested by mod ${message.senderModId}.")
      ...
      case compInfo: CompoundTag if message.method == api.IMC.BLACKLIST_HOST => {
        OpenComputersNeo.log.debug(s"Blacklisting component '${compInfo.getString("name")}' for host '${compInfo.getString("host")}' as requested by mod ${message.senderModId()}.")
        ...
        try Registry.blacklistHost(stack, Class.forName(compInfo.getString("host"))) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed blacklisting component.", t)
        }
      }
      case name: String if message.method == api.IMC.REGISTER_ASSEMBLER_FILTER => {
        OpenComputersNeo.log.debug(s"Registering new assembler template filter '$name' from mod ${message.senderModId}.")
        try AssemblerTemplates.addFilter(name) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering assembler template filter.", t)
        }
      }
      case name: String if message.method == api.IMC.REGISTER_INK_PROVIDER => {
        OpenComputersNeo.log.debug(s"Registering new ink provider '${name}' from mod ${message.senderModId}.")
        try PrintData.addInkProvider(getStaticMethod(name, classOf[ItemStack])) catch {
          case t: Throwable => OpenComputersNeo.log.warn("Failed registering ink provider.", t)
        }
      }
      case diskInfo: CompoundTag if message.method == api.IMC.REGISTER_PROGRAM_DISK_LABEL => {
        OpenComputersNeo.log.debug(s"Registering new program location mapping for program '${diskInfo.getString("program")}' being on disk '${diskInfo.getString("label")}' from mod ${message.senderModId}.")
      ...
      case _ => OpenComputersNeo.log.warn(s"Got an unrecognized or invalid IMC message '${message.method}' from mod ${message.senderModId}.")
    }
  }
  ...
  def tryInvokeStatic[T](method: Method, args: AnyRef*)(default: T): T = try method.invoke(null, args: _*).asInstanceOf[T] catch {
    case t: Throwable =>
      OpenComputersNeo.log.warn(s"Error invoking callback ${method.getDeclaringClass.getCanonicalName + "." + method.getName}.", t)
      default
  }
