...
object Loot {
  ...
  def isLootDisk(stack: ItemStack): Boolean = api.Items.get(stack) == api.Items.get(Constants.ItemName.Floppy) && stack.hasTagCompound && stack.getTagCompound.contains(Settings.namespace + "lootFactory", NBT.TAG_STRING)
