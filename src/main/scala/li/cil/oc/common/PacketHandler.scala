...
abstract class PacketHandler {
  ...
  protected def onPacketData(data: ByteBuf, player: Player): Unit = {
    ...
    player match {
      case mp: ServerPlayer => mp.func_143004_u()
      case _ => // Uh... OK?
    }
  }
  ...
  /**
    ...
    */
  protected def world(player: Player, dimension: Int): Option[Level]
  ...
  protected class PacketParser(stream: InputStream, val player: Player) extends DataInputStream(stream) {
    ...
    def readDirection() = readByte() match {
      ...
      case id => Option(Direction.getOrientation(id))
    }
