...
class Disassembler extends SimpleBlock with traits.PowerAcceptor with traits.StateAware with traits.GUI {
  ...
  override protected def tooltipBody(metadata: Int, stack: ItemStack, player: Player, tooltip: java.util.List[String], advanced: Boolean): Unit = {
  ...
  override def createTileEntity(world: Level, metadata: Int) = new tileentity.Disassembler()
}
