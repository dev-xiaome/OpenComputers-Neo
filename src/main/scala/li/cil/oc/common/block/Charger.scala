...
class Charger extends RedstoneAware with traits.PowerAcceptor with traits.StateAware with traits.GUI {
  ...
  override def createTileEntity(world: Level, metadata: Int) = new tileentity.Charger()

  // ----------------------------------------------------------------------- //

  override def canConnectRedstone(world: IBlockAccess, x: Int, y: Int, z: Int, side: Direction) = true
