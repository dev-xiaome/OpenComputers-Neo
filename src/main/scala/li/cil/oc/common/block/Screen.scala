...
class Screen(val tier: Int) extends RedstoneAware {
    ...
    world.getTileEntity(x, y, z) match {
      case screen: tileentity.Screen if screen.width > 1 || screen.height > 1 =>
        ...
        localSide match {
          case Direction.SOUTH | Direction.NORTH =>
            val (ud, sn, ew) =
              if (localSide == Direction.SOUTH) (Icons.fud, Icons.fsn, Icons.few)
          ...
          case Direction.EAST | Direction.WEST =>
            val (ud, sn, ew) =
              if (localSide == Direction.EAST) (Icons.sud, Icons.sse, Icons.snw)
          ...
          case Direction.UP | Direction.DOWN =>
            val (sn, ew) =
              if (localSide == Direction.UP ^ screen.pitch == Direction.DOWN) (Icons.snw, Icons.sse)
              else (Icons.sse, Icons.snw)
            val Array(t, m, b) = screen.pitch match {
              case Direction.NORTH => screen.yaw match {
                case Direction.SOUTH => Icons.th
                case Direction.NORTH => Icons.bh
                case Direction.EAST => Icons.bv
                case Direction.WEST => Icons.tv
                case _ => throw new AssertionError("yaw has invalid value")
              }
              case _ => screen.yaw match {
            ...
            if (screen.width == 1) {
              if (screen.pitch == Direction.NORTH) Icons.b
              else Icons.b2
            }
            else {
              if (lx == 0) b
      ...
      case screen: tileentity.Screen =>
        val (f, b, t, s) = screen.pitch match {
          case Direction.NORTH => (Icons.f2, Icons.b2, Icons.b, Icons.b2)
          case _ => (Icons.f, Icons.b, Icons.b2, Icons.b2)
        }
        localSide match {
          case Direction.SOUTH => f
          case Direction.NORTH => b
          case Direction.DOWN | Direction.UP => t
          case _ => s
        }
      case _ => getIcon(localSide, 0)
    }

  override def getIcon(side: Direction, metadata: Int) =
    side match {
      case Direction.SOUTH => Icons.f2
      case Direction.DOWN | Direction.UP => Icons.b
      case _ => Icons.b2
    }
  ...
  override def createTileEntity(world: Level, metadata: Int) = new tileentity.Screen(tier)
  ...
  override def onBlockActivated(world: Level, x: Int, y: Int, z: Int, player: Player,
                                side: Direction, hitX: Float, hitY: Float, hitZ: Float) = rightClick(world, x, y, z, player, side, hitX, hitY, hitZ, force = false)

  def rightClick(world: Level, x: Int, y: Int, z: Int, player: Player,
  ...
  override def onEntityWalking(world: Level, x: Int, y: Int, z: Int, entity: Entity) =
  ...
  override def onEntityCollidedWithBlock(world: Level, x: Int, y: Int, z: Int, entity: Entity) =
  ...
  override def getValidRotations(world: Level, x: Int, y: Int, z: Int) =
    world.getTileEntity(x, y, z) match {
      case screen: tileentity.Screen =>
        if (screen.facing == Direction.UP || screen.facing == Direction.DOWN) Direction.VALID_DIRECTIONS
        else Direction.VALID_DIRECTIONS.filter {
