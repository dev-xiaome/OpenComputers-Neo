...
class RobotProxy extends RedstoneAware with traits.SpecialBlock with traits.StateAware {
  ...
  override protected def tooltipHead(metadata: Int, stack: ItemStack, player: Player, tooltip: util.List[String], advanced: Boolean): Unit = {
  ...
  private def addLines(stack: ItemStack, tooltip: util.List[String]): Unit = {
    if (stack.hasTagCompound) {
      if (stack.getTagCompound.contains(Settings.namespace + "xp")) {
      ...
      if (stack.getTagCompound.contains(Settings.namespace + "storedEnergy")) {
  ...
  override def createTileEntity(world: Level, metadata: Int) = {
    moving.get match {
  ...
  override def getDrops(world: Level, x: Int, y: Int, z: Int, metadata: Int, fortune: Int) = {
  ...
  override def intersect(world: Level, x: Int, y: Int, z: Int, start: Vec3, end: Vec3) = {
  ...
  override def doSetBlockBoundsBasedOnState(world: IBlockAccess, x: Int, y: Int, z: Int): Unit = {
    world.getTileEntity(x, y, z) match {
      case proxy: tileentity.RobotProxy =>
        val robot = proxy.robot
        val bounds = AABB.getBoundingBox(0.1, 0.1, 0.1, 0.9, 0.9, 0.9)
  ...
  override def onBlockPlacedBy(world: Level, x: Int, y: Int, z: Int, entity: LivingEntity, stack: ItemStack): Unit = {
