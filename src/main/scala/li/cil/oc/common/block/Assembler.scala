...
class Assembler extends SimpleBlock with traits.SpecialBlock with traits.PowerAcceptor with traits.StateAware with traits.GUI {
  ...
  override def isBlockSolid(world: IBlockAccess, x: Int, y: Int, z: Int, side: Direction) = side == Direction.DOWN || side == Direction.UP

  override def isSideSolid(world: IBlockAccess, x: Int, y: Int, z: Int, side: Direction) = side == Direction.DOWN || side == Direction.UP
  ...
  override def createTileEntity(world: Level, metadata: Int) = new tileentity.Assembler()
}
