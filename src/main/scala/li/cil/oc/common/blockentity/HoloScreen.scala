...
class HoloScreen(pos: BlockPos, state: BlockState, tier: Int) extends Screen(pos, state, tier) with traits.Inventory with MenuProvider {
  ...
  override def saveForServer(nbt: CompoundTag): Unit = {
    ...
    saveSize(nbt)
    internalKeyboard.foreach(keyboard => nbt.setNewCompoundTag(KeyboardTag, keyboard.saveData))
  }
  ...
  override def saveForClient(nbt: CompoundTag): Unit = {
    ...
    saveSize(nbt)
    nbt.putBoolean(HasInternalKeyboardTag, !getItem(KeyboardSlot).isEmpty)
  }
