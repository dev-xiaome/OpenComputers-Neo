...
class Proxy(val modBus: IEventBus) {
  ...
  def preInit(): Unit = {
    OpenComputersNeo.log.info("Initializing OpenComputersNeo API.")
  ...
  def init(e: FMLCommonSetupEvent): Unit = {
    e.enqueueWork((() => {
      ...
      OpenComputersNeo.log.debug("Initializing mod integration.")
      Mods.init()
  ...
  def registerPacket(event: RegisterPayloadHandlersEvent): Unit = {
    val registrar: PayloadRegistrar = event.registrar(OpenComputersNeo.ID).versioned("1")
