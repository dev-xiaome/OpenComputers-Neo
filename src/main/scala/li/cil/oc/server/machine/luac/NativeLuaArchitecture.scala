...
abstract class NativeLuaArchitecture(val machine: api.machine.Machine) extends Architecture {
  ...
  override def runThreaded(isSynchronizedReturn: Boolean): ExecutionResult = {
    try {
      ...
      case e: LuaRuntimeException =>
        OpenComputersNeo.log.warn("Kernel crashed. This is a bug!\n" + e.toString + "\tat " + e.getLuaStackTrace.mkString("\n\tat "))
  ...
  override def loadData(nbt: CompoundTag): Unit = {
    ...
    try {
      ...
      try lua.gc(LuaState.GcAction.COLLECT, 0) catch {
        case t: Throwable =>
          OpenComputersNeo.log.warn(s"Error cleaning up loaded computer @ ${machine.host().machinePosition()}. This either means the server is badly overloaded or a user created an evil __gc method, accidentally or not.")
  ...
  override def saveData(nbt: CompoundTag): Unit = {
    ...
    try {
      ...
      case e: LuaRuntimeException =>
        OpenComputersNeo.log.warn(s"Could not persist computer @ ${machine.host().machinePosition()}.\n${e.toString}" + (if (e.getLuaStackTrace.isEmpty) "" else "\tat " + e.getLuaStackTrace.mkString("\n\tat ")))
        nbt.remove("state")
      case e: LuaGcMetamethodException =>
        OpenComputersNeo.log.warn(s"Could not persist computer @ ${machine.host().machinePosition()}.\n${e.toString}")
        nbt.remove("state")
