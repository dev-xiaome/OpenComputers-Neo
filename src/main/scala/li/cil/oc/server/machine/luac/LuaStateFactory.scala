...
abstract class LuaStateFactory {
  ...
  def init(): Unit = {
    ...
    if (SystemUtils.IS_OS_WINDOWS && !Settings.get.alwaysTryNative) {
      if (SystemUtils.IS_OS_WINDOWS_XP) {
        OpenComputersNeo.log.warn("Sorry, but Windows XP isn't supported. I'm afraid you'll have to use a newer Windows. I very much recommend upgrading your Windows, anyway, since Microsoft has stopped supporting Windows XP in April 2014.")
        return
      }

      if (SystemUtils.IS_OS_WINDOWS_2003) {
        OpenComputersNeo.log.warn("Sorry, but Windows Server 2003 isn't supported. I'm afraid you'll have to use a newer Windows.")
        return
      }
    }
    ...
    if (!Strings.isNullOrEmpty(Settings.get.forceNativeLibPathFirst)) {
      ...
      if (libraryTest.canRead) {
        ...
        OpenComputersNeo.log.info(s"Found forced-path filesystem library $currentLib.")
      }
      else
        OpenComputersNeo.log.warn(s"forceNativeLibPathFirst is set, but $currentLib was not found there. Falling back to checking the built-in libraries.")
    }
    ...
    if (currentLib.isEmpty) {
      ...
      val tmpLibName = s"OpenComputersMod-${OpenComputersNeo.Version}-$version-$libraryName"
      ...
      if (tmpLibFile.exists()) {
        ...
        if (!matching) {
          ...
          if (tmpLibFile.exists()) {
            OpenComputersNeo.log.warn(s"Could not update native library '${tmpLibFile.getName}'!")
          }
        }
      }
    ...
    try {
      ...
      OpenComputersNeo.log.info(s"Found a compatible native library: '${tmpLibFile.getName}'.")
      haveNativeLibrary = true
    }
    catch {
      case t: Throwable =>
        if (Settings.get.logFullLibLoadErrors) {
          OpenComputersNeo.log.warn(s"Could not load native library '${tmpLibFile.getName}'.", t)
        }
        else {
          OpenComputersNeo.log.trace(s"Could not load native library '${tmpLibFile.getName}'.")
        }
        tmpLibFile.delete()
    }
  }
  ...
  if (!haveNativeLibrary) {
    OpenComputersNeo.log.warn("Unsupported platform, you won't be able to host games with persistent computers.")
  }
  ...
  def createState(): Option[jnlua.LuaState] = {
    ...
    try {
      ...
      try {
        ...
        case t: Throwable =>
          OpenComputersNeo.log.warn("Failed creating Lua state.", t)
          state.close()
      }
    }
    catch {
      case _: UnsatisfiedLinkError =>
        OpenComputersNeo.log.error("Failed loading the native libraries.")
      case t: Throwable =>
        OpenComputersNeo.log.warn("Failed creating Lua state.", t)
    }
    None
