...
import net.minecraft.entity.player.Player.EnumStatus
...
class Player(val agent: internal.Agent) extends FakePlayer(agent.world.asInstanceOf[ServerLevel], Player.profileFor(agent)) {
...

  {
    ...
    if (Mods.BattleGear2.isAvailable) {
      ObfuscationReflectionHelper.setPrivateValue(classOf[Player], this, inventory, "inventory", "field_71071_by", "bm")
    }
  ...
  def closestEntity[Type <: Entity : ClassTag](side: Direction = facing) = {
  ...
  private def adjacentItems = {
    world.getEntitiesWithinAABB(classOf[ItemEntity], BlockPosition(agent).bounds.expand(2, 2, 2)).map(_.asInstanceOf[ItemEntity])
  }
  ...
  override def attackTargetEntityWithCurrentItem(entity: Entity): Unit = {
    callUsingItemInSlot(agent.equipmentInventory, 0, stack => entity match {
      case player: Player if !canAttackPlayer(player) => // Avoid player damage.
      case _ =>
  ...
  override def interactWith(entity: Entity) = {
    ...
    !cancel && callUsingItemInSlot(agent.equipmentInventory, 0, stack => {
      val result = isItemUseAllowed(stack) && (entity.interactFirst(this) || (entity match {
        case living: LivingEntity if getHeldItem != null => getHeldItem.interactWithEntity(this, living)
        case _ => false
      }))
  ...
  override def canAttackPlayer(player: Player) = Settings.get.canAttackPlayers
  ...
  override def setRevengeTarget(entity: LivingEntity) {}
  ...
  override def addChatMessage(message: Component) {}
