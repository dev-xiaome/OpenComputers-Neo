    ...
    player.pick(MaxDistance, 0.0f, false) match {
      case hit: BlockHitResult if hit.getType == HitResult.Type.BLOCK =>
        ...
        if (components.size != componentNames.size + 2 || components.exists(_.isEmpty)) {
          source.sendFailure(Component.literal("OpenComputersNeo default EEPROM/OpenOS data is not loaded; reload the server resources and try again."))
          return 0
        }
        ...
        source.sendSuccess(() => Component.literal("Spawned a configured OpenComputersNeo computer."), true)
        1
      case _ =>
