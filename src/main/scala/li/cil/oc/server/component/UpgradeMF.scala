...
class UpgradeMF(val host: EnvironmentHost, val coord: BlockPosition, val dir: Direction) extends AbstractManagedEnvironment with ChangeListener with DeviceInfo {
  ...
  private def otherNode(tile: BlockEntity, f: (Node) => Unit): Unit = {
  ...
  private def updateBoundState(): Unit = {
    if (node != null && node.network != null && coord.world.exists(_.dimension == host.getEnvironmentLevel.dimension)
      && coord.toVec3.distanceTo(new Vec3(host.xPosition, host.yPosition, host.zPosition)) <= Settings.get.mfuRange) {
      host.getEnvironmentLevel.getBlockEntity(coord) match {
