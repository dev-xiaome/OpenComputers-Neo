...
trait Agent extends traits.WorldControl with traits.InventoryControl with traits.InventoryWorldControl with traits.TankAware with traits.TankControl with traits.TankWorldControl {
  ...
  protected def rotatedPlayer(facing: Direction = agent.facing, side: Direction = agent.facing) = {
  ...
  def swing(context: Context, args: Arguments): Array[AnyRef] = {
    ...
    val sides =
      if (args.isInteger(1)) {
        ...
        Iterable(facing) ++ Direction.VALID_DIRECTIONS.filter(side => side != facing && side != facing.getOpposite).toIterable
      }
      ...
      val (success, what) = {
        ...
        (Option(hit) match {
          ...
          case _ =>
            ...
            player.closestEntity[LivingEntity]() match {
              case Some(entity) =>
  ...
  protected def pick(player: Player, range: Double) = {
    ...
    player.closestEntity[Entity]() match {
      case Some(entity@(_: LivingEntity | _: EntityMinecart | _: entity.Drone)) if hit == null || Vec3.createVectorHelper(player.posX, player.posY, player.posZ).distanceTo(hit.hitVec) > player.getDistanceToEntity(entity) => new HitResult(entity)
      case _ => hit
    }
  }

  protected def clickParamsFromHit(hit: HitResult) = {
