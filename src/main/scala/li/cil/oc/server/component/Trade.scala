...
class Trade(val info: TradeInfo) extends AbstractValue {
  ...
  override def load(nbt: CompoundTag) = EventHandler.scheduleServer(() => info.load(nbt))
    ...
    info.merchant.get match {
      case Some(merchant) => {
        ...
        def containsAccumulativeItemStack(stack: ItemStack) =
          InventoryUtils.extractFromInventory(stack, inventory, Direction.UNKNOWN, simulate = true, exact = exact).stackSize == 0
        ...
        InventoryUtils.extractFromInventory(firstInputStack, inventory, Direction.UNKNOWN, exact = exact)
        secondInputStack.map(InventoryUtils.extractFromInventory(_, inventory, Direction.UNKNOWN, exact = exact))
...
class TradeInfo(var host: Option[EnvironmentHost], var merchant: WeakReference[IMerchant], var recipeID: Int, var merchantID: Int) {
  ...
  def load(nbt: CompoundTag): Unit = {
    ...
    merchantID = if (nbt.contains("merchantID")) nbt.getInteger("merchantID") else -1
  }
  ...
  def save(nbt: CompoundTag): Unit = {
    host match {
      case Some(entity: Entity) =>
        ...
        nbt.putInt("dimensionID", entity.world.provider.dimensionId)
        nbt.putLong("hostUUIDLeast", entity.getPersistentID.getLeastSignificantBits)
        nbt.putLong("hostUUIDMost", entity.getPersistentID.getMostSignificantBits)
      case Some(tileEntity: BlockEntity) =>
        nbt.putBoolean("hostIsEntity", false)
        nbt.putInt("dimensionID", tileEntity.getWorldObj.provider.dimensionId)
        nbt.putInt("hostX", tileEntity.xCoord)
        nbt.putInt("hostY", tileEntity.yCoord)
        nbt.putInt("hostZ", tileEntity.zCoord)
      case _ => // Welp!
    }
    merchant.get match {
      case Some(entity: Entity) =>
        nbt.putLong("merchantUUIDLeast", entity.getPersistentID.getLeastSignificantBits)
        nbt.putLong("merchantUUIDMost", entity.getPersistentID.getMostSignificantBits)
      case _ =>
    }
