...
abstract class DriverCPU extends Item with api.driver.item.MutableProcessor with api.driver.item.CallBudget {
    ...
    if (stack.hasTagCompound) {
      ...
      if (!archClass.isEmpty) try return Class.forName(archClass).asSubclass(classOf[api.machine.Architecture]) catch {
        case t: Throwable =>
          ...
          stack.getTagCompound.remove(Settings.namespace + "archClass")
          stack.getTagCompound.remove(Settings.namespace + "archName")
      }
    }
  ...
  override def setArchitecture(stack: ItemStack, architecture: Class[_ <: api.machine.Architecture]): Unit = {
    ...
    if (!stack.hasTagCompound) stack.put(new CompoundTag())
    stack.getTagCompound.putString(Settings.namespace + "archClass", architecture.getName)
    stack.getTagCompound.putString(Settings.namespace + "archName", api.Machine.getArchitectureName(architecture))
  }
