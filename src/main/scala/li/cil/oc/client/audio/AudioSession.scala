...
class AudioSession(
  ...
  def play(): Unit = {
    ...
    OpenComputersNeo.log.info(s"Audio stream play: handle=$handle, sampleRate=$sampleRate, format=$format, pcmBytes=$appendedBytes, pcmSha256=$pcmSha256, buffers=${pendingBuffers.size}")
    ...
    val queued = Audio.runOnSoundEngine {
      try {
        ...
        case t: Throwable =>
          OpenComputersNeo.log.error("Failed to play audio", t)
