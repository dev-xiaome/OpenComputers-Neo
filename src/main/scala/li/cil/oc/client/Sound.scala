...
object Sound {
  ...
  private val updateTimer = new Timer("OpenComputersNeo-SoundUpdater", true)
      ...
      commandQueue.synchronized {
        while (commandQueue.nonEmpty && commandQueue.head.when < System.currentTimeMillis()) {
          try commandQueue.dequeue()() catch {
            case t: Throwable => OpenComputersNeo.log.warn("Error processing sound command.", t)
          }
        }
      }
    }
  ...
  private class PseudoLoopingStream(val blockEntity: WeakReference[BlockEntity], val subVolume: Float, name: String)
    extends AbstractSoundInstance(ResourceLocation.fromNamespaceAndPath(OpenComputersNeo.ID, name), SoundSource.BLOCKS, SoundInstance.createUnseededRandom()) with TickableSoundInstance {
