  ...
  override def renderItem(renderType: ItemRenderType, stack: ItemStack, data: AnyRef*): Unit = {
    ...
    if (isFloppy(descriptor)) {
      ...
      if (fontRenderer != null && res.getScaleFactor > 1) {
        ...
        val label =
          if (stack.hasTagCompound && stack.getTagCompound.contains(Settings.namespace + "data") && stack.getTagCompound.getCompound(Settings.namespace + "data").contains(Settings.namespace + "fs.label")) {
            stack.getTagCompound.getCompound(Settings.namespace + "data").getString(Settings.namespace + "fs.label")
          }
          else "disk"
        val lines = fontRenderer.listFormattedStringToWidth(ChatFormatting.getTextWithoutFormattingCodes(label), maxLength).take(math.max(1, res.getScaleFactor / 2))
        for (line <- lines) {
