...
object ScreenRenderer extends TileEntitySpecialRenderer {
  ...
  override def renderTileEntityAt(t: BlockEntity, x: Double, y: Double, z: Double, f: Float): Unit = {
  ...
  private def drawOverlay() = if (screen.facing == Direction.UP || screen.facing == Direction.DOWN) {
