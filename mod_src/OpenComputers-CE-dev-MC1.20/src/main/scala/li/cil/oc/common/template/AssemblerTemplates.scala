...
object AssemblerTemplates {
  val NoSlot = new AssemblerSlot(Slot.None, Tier.None, None, None)
  ...
  class Template(val selector: Method,
                 ...
                 val containerSlots: Array[AssemblerSlot],
                 val upgradeSlots: Array[AssemblerSlot],
                 val componentSlots: Array[AssemblerSlot]) {
  ...
  class AssemblerSlot(val kind: String, val tier: Int, val validator: Option[Method], val hostClass: Option[Class[_ <: EnvironmentHost]]) {
  ...
  private def parseSlot(nbt: CompoundTag, kindOverride: Option[String], hostClass: Option[Class[_ <: EnvironmentHost]]) = {
    ...
    new AssemblerSlot(kind, tier, validator, hostClass)
  }
